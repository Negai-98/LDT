...
def sample_mask(sample_sizes, max_size):
    """
    :param sample_sizes: Tensor([B, N])
    :param max_size:
    :return: mask BoolTensor([B, N])
    """
    presence = [torch.randperm(max_size) < sample_sizes[1] for _ in range(sample_sizes[0])]
    mask = ~torch.stack(presence, dim=0)
    return mask.bool()


def get_mask(sizes, max_size):
    """
    :param sizes: Tensor([B, N])
    :param max_size:
    :return: mask BoolTensor([B, N])
    """
    presence = [torch.arange(max_size) < sizes[1] for _ in range(sizes[0])]
    mask = ~torch.stack(presence, dim=0)
    return mask.bool()
