...
class ViPCDataLoader(Dataset):
    def __init__(self, filepath, data_path, status, pc_input_num=3500, view_align=False, category='all', preload=True):
        ...
        if preload:
            ...
            for key in tbar:
                pc_part_path = os.path.join(self.imcomplete_path,
                                            key.split(';')[0] + '/' + key.split(';')[1] + '/' + key.split(';')[
                                                -1].replace('\n',
                                                            '') + '.dat')

                file_name = key.split(';')[1]
                if file_name in self.all_pc_gt or file_name in self.all_pc_view:
                    continue
                else:
                    self.all_pc_gt[file_name] = []
                    self.all_pc_view[file_name] = []
                for i in range(24):
                    ran_key = key[:-3] + str(i).rjust(2, '0')
                    pc_path = os.path.join(self.gt_path,
                                           ran_key.split(';')[0] + '/' + ran_key.split(';')[1] + '/' +
                                           ran_key.split(';')[
                                               -1].replace('\n', '') + '.dat')
                    ...
                    views = views[:3, :, :]
                    self.all_pc_view[file_name].append(views)
                    if not os.path.exists(pc_path):
                        print(pc_path + 'missing')
                    else:
                        with open(pc_path, 'rb') as f:
                            pc = pickle.load(f).astype(np.float32)
                            self.all_pc_gt[file_name].append(pc)




        # self check
        # for key in tqdm(self.key):
        #     pc_part_path = os.path.join(self.imcomplete_path,
        ...
        #                              ran_key.split(';')[0] + '/' + ran_key.split(';')[1] + '/rendering/' +
        #                              ran_key.split(';')[-1].replace('\n', '') + '.png')
        #     if not os.path.exists(pc_part_path):
        #         print(pc_part_path + 'missing')
        #     if not os.path.exists(pc_path):
        #         print(pc_part_path + 'missing')
    ...
    def __getitem__(self, idx):
        ...
        if self.preload:
            ...
            while flag:
                ...
                ran_key = random.randint(0, 23)
                if len(self.all_pc_gt[filename]) > ran_key:
                    pc = self.all_pc_gt[filename][ran_key]
                else:
                    continue
                if len(self.all_pc_gt[filename]) > ran_key:
                    views = self.all_pc_view[filename][ran_key]
                else:
                    continue
                if not os.path.exists(pc_part_path):
                    print(pc_part_path + 'missing')
                ...
                ran_key = key[:-3] + str(ran_key).rjust(2, '0')
                pc_path = os.path.join(self.gt_path,
                                       ran_key.split(';')[0] + '/' + ran_key.split(';')[1] + '/' + ran_key.split(';')[
                                           -1].replace('\n', '') + '.dat')
                view_path = os.path.join(self.rendering_path,
        ...
        pc_part = torch.from_numpy(pc_part).float()
        pc = torch.from_numpy(pc).float()
        # shift, scale = gt_mean, pc_L_max
        return views.float(), pc, pc_part
...
def get_data_loaders(cfg):
    tr_dataset = ViPCDataLoader('datasets/ViPC/train_list2.txt', cfg.data_dir, status="train", category=cfg.train_cate, preload=cfg.train_preload)
    te_dataset = ViPCDataLoader('datasets/ViPC/test_list2.txt', cfg.data_dir, status="test", category=cfg.test_cate, preload=cfg.test_preload)
